...
double dFactorial2(const int64_t & N) {
    if (N < 2) return 1.0;
        ...
        default:
            if(N % 2 ==1) return dFactorial(N + 1) / (pow(2, (N + 1) / 2) * dFactorial((N + 1) / 2));
            else return pow(2, N / 2) * dFactorial(N / 2);
    }
}
...
double dPermutation(const size_t & M, const size_t & N) {
    if (M < 2 || N == 0) return 1.0;
    else if (N == 1) return (double)M;
    else if (N == M || N == M - 1) return dFactorial(M);
    else switch (M) {
        case 4: return 12.0;
        case 5:
            switch (N) {
        ...
        default:
            ...
            else return dFactorial(M) / dFactorial(M - N);
    }
}
...
double dCombination(const size_t & M, const size_t & N) {
    if (M < 2 || N == 0 || N == M) return 1.0;
    else if (N == 1 || N == M - 1) return (double)M;
    else switch (M) {
        case 4: return 6.0;
        case 5: return 10.0;
        default:
...
unsigned long long iPermutation(const size_t & M, const size_t & N) {
    if (M < 2 || N == 0) return 1;
    ...
    else if (N == M || N == M - 1) return iFactorial(M);
    else switch (M) {
        case 4: return 12;
        case 5:
            switch (N) {
        ...
        default:
            ...
            else return iFactorial(M) / iFactorial(M - N);
    }
}
